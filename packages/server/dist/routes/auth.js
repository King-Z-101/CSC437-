...
__export(auth_exports, {
  authenticateZookeeper: () => authenticateZookeeper,
...
const TOKEN_SECRET = process.env.TOKEN_SECRET || "PREHISTORIC_ZOO_SECRET";
...
router.post("/login", (req, res) => {
  ...
  if (!username || !password) {
    ...
    import_credential_svc.default.verify(username, password).then((goodUser) => generateAccessToken(goodUser)).then((token) => res.status(200).send({ token })).catch((error) => res.status(401).send("Unauthorized: Invalid zookeeper credentials"));
  }
});
function authenticateZookeeper(req, res, next) {
